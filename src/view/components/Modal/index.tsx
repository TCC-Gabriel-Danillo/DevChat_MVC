...
export function Modal({ children, onCloseRequest, onPressOutside, isOpen }: Props) {
  return (
    <RnModal visible={isOpen} transparent animationType="fade" onRequestClose={() => onCloseRequest?.()}>
      <TouchableOpacity activeOpacity={1} style={styles.container} onPress={() => onPressOutside?.()}>
